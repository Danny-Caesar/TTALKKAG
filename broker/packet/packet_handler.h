...
public:
    static std::vector<uint8_t> handle(const uint8_t* data, size_t size);

private:
    static std::vector<uint8_t> handle_connect(connect_packet& packet);
    static std::vector<uint8_t> handle_publish(publish_packet& packet);
    static std::vector<uint8_t> handle_subscribe(subscribe_packet& packet);
};
